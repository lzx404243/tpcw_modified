...
public class TPCW_Database {
    ...
    static String jdbcPath = "jdbc:mysql://localhost:3306/tpcw?user=root&password=TigerBit!2016";
	...
	try {
	    ...
	    PreparedStatement statement = con.prepareStatement
		("SELECT i_id, i_title, a_fname, a_lname " +"FROM item, author, order_line " +"WHERE item.i_id = order_line.ol_i_id " +"AND item.i_a_id = author.a_id " +"AND item.i_subject = ? " +"GROUP BY i_id, i_title, a_fname, a_lname " +"ORDER BY SUM(ol_qty) DESC " +"limit 10");
